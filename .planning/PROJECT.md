...
- ✓ **自更新与技能自动重注入机制 (yongle-update)** — v1.5 Upgrade
...
- **持久化方案**: 本地以 Markdown 文本为主，底层引入 SQLite 与 LanceDB 二维数据库引擎以提供极速的混合 FTS + 语义搜索。
...
| Git/npm 双轨检测与智能依赖更新 | D-01/D-04: 支持根据本地仓库的类型自适应进行 Git 增量拉取（并比对 MD5 跳过冗余安装）或 npm 全局升级，保障不同场景下的升级流畅与自愈回滚。 | ✓ Good |
| 自动扫描 install-manifest.json 重注入 | D-08/D-09: 扫描各个主流运行时的凭证并自动触发 `bin/install.js` 重注入对应运行时的技能，彻底免去升级后的手工重复配置动作。 | ✓ Good |
...
---
*Last updated: 2026-05-24 after Milestone v1.5 Upgrade completion*
